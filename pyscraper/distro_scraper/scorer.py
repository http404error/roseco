...
def getGitUsersActivity(user_ids, cur):
	with open(os.path.join('score_config','git_activity.yaml'),'r') as config_file:
		config = yaml.load(config_file)

	user_impact = scoreGitUsersActivity(user_ids, config['impact'], cur)

	return user_impact

def getImpactivity(git_ppl_ids, cur):
	with open(os.path.join('score_config','git_ppl_impactivity.yaml'),'r') as config_file:
		config = yaml.load(config_file)

	impactivity = scoreImpactivity(git_ppl_ids, config['impactivity'], cur)
...
def scorePkgsHealth(pkg_ids, health_config, cur):
	...
	for metric_config in health_config:
		...
		elif metric_config['metric'] == 'maintainer_email' and include:
...
def scoreReposHealth(repo_ids, health_config, cur):
	...
	for metric_config in health_config:
		...
		elif metric_config['metric'] == 'github_issues' and include:
...
def scorePplImpact(ppl_ids, impact_config, cur):
	...
	for metric_config in impact_config:
		...
		elif metric_config['metric'] == 'pkg_impact':
...
def scoreGitUsersActivity(user_ids, impact_config, cur):
	impact_scores = []
	for user_id in user_ids:
		impact_scores.append([user_id[0],0.0])

	for metric_config in impact_config:
		include = metric_config['include']
		weights = metric_config['weights']
		if metric_config['metric'] == 'created_issues':
			impact_scores = scoreGitUserCreated(impact_scores, weights, cur)
		elif metric_config['metric'] == 'assigned_issues':
			impact_scores = scoreGitUserAssigned(impact_scores, weights, cur)

	return makeNormalizedLot(impact_scores)

def scoreImpactivity(git_ppl_ids, impactivity_config, cur):
	impactivity_scores = []
	for gp_id in git_ppl_ids:
		impactivity_scores.append([gp_id[0], 0.0])

	for metric_config in impactivity_config:
		include = metric_config['include']
		weights = metric_config['weights']
		if metric_config['metric'] == 'git_activity_ppl_impact_composite':
			impactivity_scores = scoreGitPplComp(impactivity_scores, weights, cur)
	
	return makeNormalizedLot(impactivity_scores)
...
def scorePkgsMntnrNum(health_scores, weights, cur):
	...
	for score in health_scores:
		pkg_id = score[0]
		mntnr_num = dbe.getMatch(cur,table='PkgMntnrs',cols=['ppl_id'],mcol='pkg_id',val=pkg_id,cnt=True)
...
def scorePkgsMntnrEmail(health_scores, weights, cur):
	for score in health_scores:
		pkg_id = score[0]
		pkg_mntnrs = dbe.getMatch(cur,table='PkgMntnrs',cols=['ppl_id'],mcol='pkg_id',val=pkg_id)
		for pkg_mntnr in pkg_mntnrs:
			emails = dbe.getMatch(cur,table='People',cols=['email'],mcol='id',val=pkg_mntnr[0])[0] # Index bc a nested tuple
			for email in emails:
				if email == None:
					continue
				for key in weights:
...
def scoreReposInfo(health_scores, weights, cur):
	for score in health_scores:
		...
		repo_info = dbe.getMatch(cur,table='Repos',cols=cols,mcol='id',val=repo_id)[0] # Know there's only 1 row per repo id
...
def scoreReposIssues(health_scores, weights, cur):
	...
	for score in health_scores:
		...
		issues_info = dbe.getMatch(cur,table='Issues',cols=cols,mcol='repo_id',val=repo_id)
		for issue in issues_info:
			...
			if closed == False:
				...
				issue_id = issue[0]
				label_ids = dbe.getMatch(cur,table='IssuesLabels',cols=['label_id'],mcol='issue_id',val=issue_id)
...
def scorePkgsReposHealthComp(health_scores, weights, cur):
	...
	for score in health_scores:
		pkg_id = score[0]
		pkg_health = dbe.getMatch(cur,table='PkgHealth',cols=['health'],mcol='pkg_id',val=pkg_id)[0][0]
		repo_ids = dbe.getMatch(cur,table='PkgRepos',cols=['repo_id'],mcol='pkg_id',val=pkg_id)
		if repo_ids != None:
			# Pkgs can be linked to more than one (release or source) repo
			score[1] += pkg_health * pkg_health_weight
			for repo_id in repo_ids:
				repo_health = dbe.getMatch(cur,table='RepoHealth',cols=['health'],mcol='repo_id',val=repo_id[0])[0][0]
...
def scorePplPkgNum(impact_scores, weights, cur):
	...
	for score in impact_scores:
		ppl_id = score[0]
		mntning_num = dbe.getMatch(cur,table='PkgMntnrs',cols=['pkg_id'],mcol='ppl_id',val=ppl_id,cnt=True)
		authored_num = dbe.getMatch(cur,table='PkgAuthors',cols=['pkg_id'],mcol='ppl_id',val=ppl_id,cnt=True)
...
def scorePplPkgImpact(impact_scores, weights, cur):
	...
	for score in impact_scores:
		...
		pkgs_mntning = dbe.getMatch(cur,table='PkgMntnrs',cols=['pkg_id'],mcol='ppl_id',val=ppl_id)
		if len(pkgs_mntning) != 0:
			for pkg_mntning in pkgs_mntning:
				pkg_id = pkg_mntning[0]
				pkg_impact = dbe.getMatch(cur,table='PkgImpact',cols=['impact'],mcol='pkg_id',val=pkg_id)[0][0]
				score[1] += pkg_impact * weights['maintaining']
		
		pkgs_authored = dbe.getMatch(cur,table='PkgAuthors',cols=['pkg_id'],mcol='ppl_id',val=ppl_id)
...
def scoreGitUserCreated(impact_scores, weights, cur):
	# Number of issues created by the users, adjusted for recency
	for score in impact_scores:
		name = dbe.getMatchVal(cur,'GitUsers','username','id',score[0])
		issues = dbe.getMatch(cur,'Issues',['created_at'],'created_by',name)
		for issue in issues:
			ds = daysAgo(issue[0])
			if ds != 0: score[1] += weights['created_at'] / ds
			else: score[1] += weights['created_at']

	return impact_scores

def scoreGitUserAssigned(impact_scores, weights, cur):
	# Number of issues created by the users, adjusted for recency
	for score in impact_scores:
		name = dbe.getMatchVal(cur,'GitUsers','username','id',score[0])
		issues = dbe.getMatch(cur,'Issues',['created_at'],'assigned_to',name)
		for issue in issues:
			ds = daysAgo(issue[0])
			if ds != 0: score[1] += weights['assigned_to'] / ds
			else: score[1] += weights['assigned_to']

	return impact_scores

def scoreGitPplComp(impactivity_scores, weights, cur):
	git_activity_weight = weights['git_activity']
	ppl_impact_weight = weights['ppl_impact']
	
	for score in impactivity_scores:
		git_id = dbe.getMatchVal(cur,'GitPpl','git_id','id',score[0])
		ppl_id = dbe.getMatchVal(cur,'GitPpl','ppl_id','id',score[0])
		git_activity = dbe.getMatchVal(cur,'GitActivity','activity','git_id',git_id)
		ppl_impact = dbe.getMatchVal(cur,'PplImpact','impact','ppl_id',ppl_id)
		score[1] += git_activity * git_activity_weight
		score[1] += ppl_impact * ppl_impact_weight
		
	return impactivity_scores
