...
def getPplInfo(packages, people, info):
	...
	for idx,p in enumerate(people, start=1):
		...
		for i in info:
			if i == 'emails':
				emails = [eval('p.'+i)]
				if len(emails) == 0: l.append(None) # empty set means no email
				else: l.append(emails[0]) # append the first email for nomalized data & everyone only has 1 email currently
			else:
				l.append(eval('p.'+i))
		lot.append(tuple(l))
	return lot
...
def getEmailsLinkPpl(people, ppl_ids):
	lot = []
	c = 1
