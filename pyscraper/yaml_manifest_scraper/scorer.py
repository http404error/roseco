...
def getPkgsHealth(pkg_ids, cur):
	...
	with open('pkg_health.yaml', 'r') as config_file:
...
def getPkgsImpact(pkg_ids, cur):
	with open('pkg_impact.yaml', 'r') as config_file:
		config = yaml.load(config_file)
...
def getReposHealth(repo_ids, cur):
	with open('repo_health.yaml', 'r') as config_file:
		config = yaml.load(config_file)
...
def getPplImpact(ppl_ids, cur):
	with open('ppl_impact.yaml', 'r') as config_file:
		config = yaml.load(config_file)
...
def getPkgsReposHealth(pkg_ids, repo_ids, cur):
	with open('pkg_repo_health.yaml') as config_file:
		config = yaml.load(config_file)
...
def scorePkgsHealth(pkg_ids, health_config, cur):
	...
	return makeNormLotRejectOutliers(health_scores)
...
def scorePkgsImpact(pkg_ids, impact_config, cur):
	...
	return makeNormLotRejectOutliers(impact_scores)
...
def scorePkgsMntnrNum(health_scores, weights, cur):
	...
	for score in health_scores:
		pkg_id = score[0]
		mntnr_num = dbc.getMatch(cur=cur,table='PkgMntnrs',cols=['ppl_id'],match_col='pkg_id',val=pkg_id,num=True)
...
def scorePkgsMntnrEmail(health_scores, weights, cur):
	for score in health_scores:
		pkg_id = score[0]
		pkg_mntnrs = dbc.getMatch(cur=cur,table='PkgMntnrs',cols=['ppl_id'],match_col='pkg_id',val=pkg_id)
		# if len(pkg_mntnrs) == 0: continue
		for pkg_mntnr in pkg_mntnrs:
			emails = dbc.getMatch(cur=cur,table='People',cols=['email'],match_col='id',val=pkg_mntnr[0])[0] # Index bc a nested tuple
			for email in emails:
				if email == None:
					continue
				for key in weights:
...
def scoreReposInfo(health_scores, weights, cur):
	for score in health_scores:
		...
		repo_info = dbc.getMatch(cur=cur,table='Repos',cols=cols,match_col='id',val=repo_id)[0] # Know there's only 1 row per repo id
...
def scoreReposIssues(health_scores, weights, cur):
	for score in health_scores:
		repo_id = score[0]
		cols = ['closed_at','assigned_to','created_at','labels']
		issues_info = dbc.getMatch(cur=cur,table='Issues',cols=cols,match_col='repo_id',val=repo_id)
		for issue in issues_info:
			for idx,col in enumerate(cols):
				if col == 'closed_at' and issue[idx] != None:
					...
					score[1] +=  weights[col]
				elif col == 'labels' and issue[idx] != None:
					for label_weight in weights[col]:
						if label_weight in issue[idx]:
							score[1] += weights[col][label_weight]
	return health_scores
...
def scorePkgsReposHealthComp(health_scores, weights, cur):
	...
	for score in health_scores:
		pkg_id = score[0]
		pkg_health = dbc.getMatch(cur=cur,table='PkgHealth',cols=['health'],match_col='pkg_id',val=pkg_id)[0][0]
		repo_id = dbc.getMatch(cur=cur,table='Packages',cols=['repo_id'],match_col='id',val=pkg_id)[0][0]
		if repo_id != None:
			repo_health = dbc.getMatch(cur=cur,table='RepoHealth',cols=['health'],match_col='repo_id',val=repo_id)[0][0]
...
def scorePkgsDDO(impact_scores, weights, cur):
	...
	for score in impact_scores:
		pkg_id = score[0]
		depended_orders_num = dbc.getDepOrders(cur, pkg_id)
...
def scorePplPkgNum(impact_scores, weights, cur):
	...
	for score in impact_scores:
		ppl_id = score[0]
		mntning_num = dbc.getMatch(cur=cur,table='PkgMntnrs',cols=['pkg_id'],match_col='ppl_id',val=ppl_id,num=True)
		authored_num = dbc.getMatch(cur=cur,table='PkgAuthors',cols=['pkg_id'],match_col='ppl_id',val=ppl_id,num=True)
...
def scorePplPkgImpact(impact_scores, weights, cur):
	...
	for score in impact_scores:
		...
		pkgs_mntning = dbc.getMatch(cur=cur,table='PkgMntnrs',cols=['pkg_id'],match_col='ppl_id',val=ppl_id)
		if len(pkgs_mntning) != 0:
			for pkg_mntning in pkgs_mntning:
				pkg_id = pkg_mntning[0]
				pkg_impact = dbc.getMatch(cur=cur,table='PkgImpact',cols=['impact'],match_col='pkg_id',val=pkg_id)[0][0]
				score[1] += pkg_impact * weights['maintaining']
		
		pkgs_authored = dbc.getMatch(cur=cur,table='PkgAuthors',cols=['pkg_id'],match_col='ppl_id',val=ppl_id)
...
def makeNormLotRejectOutliers(lol, m=2.0):
	...
	# Other form of outliers rejection based on median

	# print 's_only', s_only
	# d = np.abs(s_only - np.median(s_only)) # Find absolute distance to median of each score
	# mdev = np.median(d) # Find the median of the absolute median distances
	# print 'mdev', mdev 
	# s = d/mdev if mdev else 0 
	# print s
	# inliers = s_only[s<m]
	# print inliers
	# if type(inliers) != type(s_only): 
	# 	div_zero = True
	# else:
	# 	max_s = max(inliers)
	# 	min_s = min(inliers)
